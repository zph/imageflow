...
static void flow_bitmap_bgra_sharpen_block_edges_x(struct flow_bitmap_bgra * im, int block_size, float pct)
{
    ...
    for (y = 0; y < sy; y++)
    {
        ...
        for (current = bytes_pp, next = bytes_pp + bytes_pp; next < stride; current = next,
            ...
            row[current + 0] = uchar_clamp_ff(left_b * c_l[weight_ix] + b * c_c[weight_ix] + row[current + bytes_pp + 0] * c_r[weight_ix]);
            row[current + 1] = uchar_clamp_ff(left_g * c_l[weight_ix] + g * c_c[weight_ix] + row[current + bytes_pp + 1] * c_r[weight_ix]);
            row[current + 2] = uchar_clamp_ff(left_r * c_l[weight_ix] + r * c_c[weight_ix] + row[current + bytes_pp + 2] * c_r[weight_ix]);
            left_b = b;
            left_g = g;
            left_r = r;
            coord ++;
